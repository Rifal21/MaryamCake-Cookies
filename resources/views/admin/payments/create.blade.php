<x-app-layout>
    @section('header_title', __('Add Payment Method'))
    ...
    <div class="max-w-4xl mx-auto">
        <div class="card-premium rounded-[2.5rem] overflow-hidden">
            <div class="px-10 py-8 border-b border-[#8B5E3C]/5 bg-gray-50/30">
                ...
                <p class="text-sm text-[#8B5E3C]/60 italic">{{ __('Configure your payment gateway information') }}</p>
            </div>

            <form action="{{ route('admin.payments.store') }}" method="POST" class="p-10 space-y-8">
                @csrf
                <div class="grid md:grid-cols-2 gap-8">
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" name="admin_fee" value="{{ old('admin_fee', 0) }}" required
                            class="w-full rounded-2xl border-[#8B5E3C]/10 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37]/5 px-6 py-4 font-bold"
                            placeholder="0">
                        @error('admin_fee')
                            <p class="text-xs text-red-500 font-bold mt-1">{{ $message }}</p>
                ...
                <div class="space-y-2">
                    ...
                    <textarea name="instructions" rows="4"
                        ...
                        placeholder="Instructions for user after checkout (e.g. Please send payment to...)">{{ old('instructions') }}</textarea>
                </div>
