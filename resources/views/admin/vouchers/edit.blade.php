<x-app-layout>
    @section('header_title', __('Edit Voucher'))
    ...
    <div class="max-w-4xl mx-auto">
        <div class="card-premium rounded-[2.5rem] overflow-hidden">
            <div class="px-10 py-8 border-b border-[#8B5E3C]/5 bg-gray-50/30">
                ...
                <p class="text-sm text-[#8B5E3C]/60 italic">{{ __('Modify your discount rules') }}</p>
            </div>

            <form action="{{ route('admin.vouchers.update', $voucher) }}" method="POST" class="p-10 space-y-8">
                @csrf
                @method('PUT')
                <div class="grid md:grid-cols-2 gap-8">
                    <div class="space-y-2">
                        ...
                        <input type="text" name="code" value="{{ old('code', $voucher->code) }}" required
                            class="w-full rounded-2xl border-[#8B5E3C]/10 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37]/5 px-6 py-4 font-bold tracking-widest uppercase">
                        @error('code')
                    ...
                    <div class="space-y-2">
                        ...
                        <input type="number" step="0.01" name="min_spend"
                            value="{{ old('min_spend', $voucher->min_spend) }}" required
                            class="w-full rounded-2xl border-[#8B5E3C]/10 focus:border-[#D4AF37] focus:ring focus:ring-[#D4AF37]/5 px-6 py-4 font-bold">
                        @error('min_spend')
                            <p class="text-xs text-red-500 font-bold mt-1">{{ $message }}</p>
                        @enderror
